...
function FPDF($orientation='P',$unit='mm',$format='A4')
{
	...
	//Page format
	##Un punto es igual a 1/72 de pulgada, 
	##es decir, cerca de 0.35 mm (siendo una pulgada 2.54 cm)
	## i.e: mm * 3.5 = puntos
	if(is_string($format))
	{
		...
		elseif($format=='gafete')
			$format=array(297,375); //W=10.4cm | H=12.8cm 
			#$format=array(312,384); //W=10.4cm | H=12.8cm 
		else
...
function Cell($w,$h=0,$txt='',$border=0,$ln=0,$align='',$fill=0,$link='')
{
	...
	if($txt!='')
	{
		...
		elseif($align=='FJ') // Forzar Justificado
        {
...
function Output($name,$ruta,$dest='')
{
	...
	switch($dest)
	{
		...
		case 'F':
			//Save to local file
			$f=fopen($ruta.$name,'wb');
			if(!$f)
				$this->Error('Unable to create output file: '.$ruta.$name);
