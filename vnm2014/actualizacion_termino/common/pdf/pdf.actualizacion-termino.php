...
class PDF extends FPDF
{
    ...
    function Header() {
        ...
        $this->Rect($x[38]-1,$y,$x[8],6,'FD');
        $this->Rect($x[47],$y,$x[7],6,'FD');
        $this->Rect($x[56]-1,$y,$x[26]-1,6,'FD');
    ...
    function Hoja1(){     
        ...
        if(!$f){
            ...
            $this->SetFont($fuente,'B',$ft2);               
            $this->Rect($x[56]-1,$y,$x[25]+1,6,'B');
            $this->Text($x[50]-1,$y+$salto,utf8_decode('TOTAL'));
            $this->Text($x[70],$y+$salto,utf8_decode($Reg)); 
            $this->SetTextColor($color1);
        }          
    }
