...
class PDF extends FPDF
{
    ...
    function Hoja1($in){
        //Variables con datos
        $nombre_completo = utf8_decode($in['nombre_completo']);
        $puesto = utf8_decode($in['puesto']);        
        $cve_elector = utf8_decode($in['cve_elector']);
        $clave = utf8_decode($in['clave']);
        $vocal_nombre = utf8_decode($in['vocal_nombre']); 
        $vocal_puesto = utf8_decode($in['vocal_puesto']); 
        $vigencia = utf8_decode($in['vigencia']);
        $hojas = '';   
        ...
        $fuente='Arial';    //Fuente
        $ft0=20;        //tamaño de fuente
        $ft1=16;        //tamaño de fuente
        $ft2=12;     //tamaño de fuente
        $ft3=10;     //tamaño de fuente
        $ft4=8;     //tamaño de fuente
        $ft5=6;     //tamaño de fuente
        $salto=4;
        $saltolinea=.5;
        ...
        //Fondo
        // $this->Image('tmp/muestra.jpg',0,0,100);
        $this->Rect(0.1,0.1,104.5,132,'B');

        //Datos
        $this->SetFont('Arial','B',$ft1);
        $this->SetTextColor($color1);
        $texto = '<b>'.utf8_decode($nombre_completo).'</b>';
        $this->Ln(37);
        $this->WriteTag($x[17],65,5,$texto,0,"C",0,0);
        $this->SetFont('Arial','B',$ft2);
        $texto = '<b>'.utf8_decode($puesto).'</b>';
        $this->Ln(1);
        $this->WriteTag($x[17],70,5,$texto,0,"C",0,0);

        $this->SetFont('Arial','B',$ft3);
        $y = $y + $salto +67; 
        $this->Text($x[30],$y,utf8_decode($clave));     
        $y = $y + $salto +5; 
        $this->Text($x[22],$y,utf8_decode($cve_elector));
        $this->SetFont('Arial','B',$ft4); 
        $y = $y + $salto +3; 
        $this->Text($x[15],$y,utf8_decode($vigencia)); 
        $this->SetFont('Arial','',$ft5);
        
        $texto = '<b>'.utf8_decode($vocal_nombre).'</b>';
        $this->Ln(43);
        $this->WriteTag($x[29],41,2,$texto,0,"C",0,0);
        $texto = '<b>'.utf8_decode($vocal_puesto).'</b>';
        $this->Ln(1);
        $this->WriteTag($x[29],41,2,$texto,0,"C",0,0);
